#!/bin/bash
# Skrypt do budowania wersji Windows za pomocą MinGW-w64 w Docker
#
...
echo "Budowanie wersji Windows (wersja: ${VERSION})..."
...
echo "Budowanie obrazu Docker (MinGW-w64)..."
docker build -f scripts/Dockerfile.windows -t bmp_to_xbpp_windows_builder .
...
docker run --rm -v "$(pwd)/release/${VERSION}:/output" bmp_to_xbpp_windows_builder cp bmp_to_xbpp.exe /output/
...
if [ ! -f "release/${VERSION}/bmp_to_xbpp.exe" ]; then
    echo "BŁĄD: Błąd podczas kopiowania pliku wykonywalnego!"
    exit 1
fi

# Test pominięty - Wine nie jest już instalowane w kontenerze
...
cd release/${VERSION}
zip bmp_to_xbpp_windows_x86.zip bmp_to_xbpp.exe sample-image.bmp FILE_ID.DIZ
...
rm bmp_to_xbpp.exe sample-image.bmp FILE_ID.DIZ
...
echo ""
echo "Budowanie wersji Windows zakończone pomyślnie!"
echo "Archiwum: release/${VERSION}/bmp_to_xbpp_windows_x86.zip"
