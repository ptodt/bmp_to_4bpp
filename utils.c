...
int write_array(uchar* packed_data, int data_size, int width, int height, const char* array_name, const char* output_path, int output_format, int use_progmem, int bits_per_pixel, int dithering_method) {
    ...
    switch (output_format) {
        case 0: // FORMAT_C_ARRAY
            result = format_c_array_write(packed_data, data_size, width, height, array_name, file, use_progmem, bits_per_pixel, dithering_method);
            break;
        case 1: // FORMAT_RAW_DATA
            result = format_raw_data_write(packed_data, data_size, file, bits_per_pixel, dithering_method);
            break;
        case 2: // FORMAT_ASSEMBLER
            result = format_assembler_write(packed_data, data_size, width, height, array_name, file, bits_per_pixel, dithering_method);
            break;
        case 3: // FORMAT_MASM_ARRAY
            result = format_masm_array_write(packed_data, data_size, width, height, array_name, file, bits_per_pixel, dithering_method);
            break;
        default:
            result = 0;
            break;
...
int format_c_array_write(uchar* packed_data, int data_size, int width, int height, const char* array_name, FILE* file, int use_progmem, int bits_per_pixel, int dithering_method) {
    ...
    if (bits_per_pixel == BITS_PER_PIXEL_1BPP) {
        ...
        fprintf(file, " (dithering: %s)", dither_name);
    }
    fprintf(file, "\n");
            ...
            for (int x = 0; x < width; x += 8) {
                ...
                for (int bit = 0; bit < 8; bit++) {
                    ...
                    if (pixel_x < width) {
                        ...
                        if (pixel_value) {
                            if (pixel_order) {
                                // Little endian: bit 0 = pierwszy piksel
                                byte_value |= (1 << bit);
                            } else {
                                // Big endian: bit 7 = pierwszy piksel
                                byte_value |= (1 << (7 - bit));
                            }
                        }
                    }
